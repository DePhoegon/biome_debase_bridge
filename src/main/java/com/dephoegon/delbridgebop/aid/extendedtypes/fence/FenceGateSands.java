package com.dephoegon.delbridgebop.aid.extendedtypes.fence;
import com.dephoegon.delbase.aid.block.stock.fenceGateBlock;

public class FenceGateSands extends fenceGateBlock {
    public FenceGateSands(Properties properties, String normToolTip, String shiftToolTip, String ctrlToolTip) {
        super(properties, normToolTip, shiftToolTip, ctrlToolTip, false);
    }
}
